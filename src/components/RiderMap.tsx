...
import { Bike, MapPin, Navigation, Compass, Shield, Map as MapIcon, Globe } from "lucide-react";
    ...
    <div className="bg-[#121620] p-4 rounded-3xl border border-gray-800 shadow-2xl relative overflow-hidden text-white">
      ...
      <div className="absolute top-4 right-4 z-10 flex gap-2">
        ...
        <button
          ...
          className="bg-[#FF6B35]/20 hover:bg-[#FF6B35]/40 backdrop-blur-md px-3 py-1.5 rounded-xl border border-[#FF6B35]/40 text-xs font-bold text-orange-400 flex items-center gap-1.5 transition-all"
        >
          {mapMode === "google" ? <MapIcon className="w-3.5 h-3.5" /> : <Globe className="w-3.5 h-3.5" />}
          <span>{mapMode === "google" ? "Vector Grid" : "Google Map"}</span>
        </button>
        
        <div className="bg-[#2D6A4F]/20 backdrop-blur-md px-3 py-1.5 rounded-xl border border-[#2D6A4F]/40 text-xs font-bold text-[#2D6A4F] flex items-center gap-1.5">
      ...
      {/* Main Map Viewer */}
      {mapMode === "google" ? (
        <div className="relative w-full aspect-[45/30] bg-[#1a1f2c] rounded-2xl border border-gray-800 mb-4 overflow-hidden shadow-inner flex items-center justify-center">
          <iframe
            ...
            frameBorder="0"
            style={{ border: 0 }}
            src={`https://maps.google.com/maps?q=${riderLat},${riderLng}&z=16&t=m&hl=en&output=embed`}
            allowFullScreen
            className="w-full h-full rounded-2xl filter brightness-95"
            referrerPolicy="no-referrer"
          />
          {/* Legend indicator */}
          <div className="absolute bottom-3 left-3 bg-black/75 backdrop-blur-md px-2.5 py-1.5 rounded-lg border border-gray-800 text-[9px] flex items-center gap-1.5 font-bold text-gray-200">
            <span className="w-1.5 h-1.5 rounded-full bg-orange-500 animate-pulse" />
            <span>Google Maps Satellite Tracker</span>
          </div>
        </div>
      ) : (
